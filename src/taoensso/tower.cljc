(ns taoensso.tower
  ...
  #?(:clj (:import
           [java.util Date Locale TimeZone Formatter]
...
#?(:clj (defprotocol IFmt (pfmt [x loc style])))
#?(:clj
...
#?(:clj (defmem- collator Collator [Loc] (Collator/getInstance Loc)))
#?(:clj
...
#?(:clj (def iso-countries (set (map (comp keyword str/lower-case) (Locale/getISOCountries)))))
...
#?(:clj (def iso-langs (set (map (comp keyword str/lower-case) (Locale/getISOLanguages)))))
...
#?(:clj (def ^:private major-tz-regex #"^(Africa|America|Asia|Atlantic|Australia|Europe|Indian|Pacific)/.*"))
...
#?(:clj
   (defmacro dict-load* "Compile-time loader."
...
#?(:clj
   (defmacro dict-compile* "Compile-time compiler."
...
(defn- make-t-uncached
  ...
  (let [{:keys [#?(:clj dictionary :cljs compiled-dictionary)
        ...
        #?(:cljs _)
        #?(:cljs (do (assert (:compiled-dictionary tconfig)  "Missing tconfig key: :compiled-dictionary")
